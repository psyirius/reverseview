...
export class BibleSearch {
    constructor(O) {
        ...
        function c() {
            showPrompt({
                title: "Filename",
                message: "Enter a filename for the search results.",
                showInput: true,
                input: '',
                onOk: onSave,
                onCancel() {}
            });
            function onSave(valueFilename) {
                const ah = I.data.length;
                ...
                aj = aj + "</TABLE>\n</BODY>";
                saveVVExportInDesktop(aj, valueFilename);
                Toast.info("Search",
                    'Search Result File "' +
                    valueFilename +
