...
function getFooter() {
    var b;
    var a = bibleVersionArray[vvConfigObj.get_version1()][3];
    var c = bibleVersionArray[vvConfigObj.get_version2()][3];
    b = a + " / " + c;
    if (a == "public") {
        a = "Public Domain";
    }
...
function setFontForList() {
    var a = bibleVersionArray[vvConfigObj.get_version1()][6];
...
function putbook() {
    ...
    var a = vvConfigObj.get_listinenglish();
    var b = booknames.length;
    for (i = 0; i < b; i++) {
        if (a) {
            document.getElementById("bookList").options[i] = new Option(default_booknames[i], i);
        }
        else {
            document.getElementById("bookList").options[i] = new Option(booknames[i], i);
        }
    }
...
function vvinit() {
    ...
    if (!firstTimeCheck()) {
        rvw.ui.Dialog.show("ReVerseVIEW", "Error loading database!");
    }
    setupLeftTabFrame();
    setupRightTabFrame();
...
function vvinit_continue() {
    ...
    setTimeout(function () {
        ...
        if (a == 1 && !firstTimeFlag) {
            ...
            if (c) {
                ...
                if (c) {
                    ...
                    rvw.ui.Dialog.show("VerseVIEW", "Bible Database update failed. Please contact verseview@yahoo.com");
                }
            }
            else {
                rvw.ui.Dialog.show("VerseVIEW", "Bible Database update failed. Please contact verseview@yahoo.com");
        ...
        generateTabContent();
        air.trace("AIR Runtime Version: ".concat(air.NativeApplication.nativeApplication.runtimeVersion));
...
function fillTabs(a) {
    ...
    if (a == "screenTab") {
        ...
        $("#thirdview_primary").prop("checked", vvConfigObj.get_svShowPrimary() == "true" ? true : false);
        $("#thirdview_secondary").prop("checked", vvConfigObj.get_svShowSecondary() == "true" ? true : false);
        $("#stageviewWindow").prop("checked", vvConfigObj.get_svWindow() == "true" ? true : false);
        $("#stageviewGreenWindow").prop("checked", vvConfigObj.get_svGreenWindow() == "true" ? true : false);
        $("#thirdview_outline").prop("checked", vvConfigObj.get_svTextOutline() == "true" ? true : false);
        $("#thirdview_shadow").prop("checked", vvConfigObj.get_svTextShadow() == "true" ? true : false);
        $("#stageSettingShowTime").prop("checked", vvConfigObj.get_svShowDate() == "true" ? true : false);
...
function fillNav() {
    ...
    var b = new icon("searchButtonID", " SEARCH ", "graphics/icon/search_32.png", "graphics/icon/search_32.png", "");
    document
    ...
    var c = new icon("nav_bibleRef_presentID", " QUICK PRESENT ", "graphics/icon/qpresent_24.png", "graphics/icon/qpresent_24.png", "");
    var a = new icon("nav_bibleRef_findID", " FIND ", "graphics/icon/search_s.png", "graphics/icon/search_s.png", "");
    document
...
function processExit() {
    if (presentWindowOpen) {
        newWindow.window.nativeWindow.removeEventListener(air.Event.CLOSE, presentWindowClosed);
        newWindow.window.nativeWindow.close();
        if (stageView && newStageWindow != null) {
            newStageWindow.window.nativeWindow.removeEventListener(air.Event.CLOSE, presentWindowClosed);
